...
const uint8_t * find_color_begin(const std::vector<uint8_t>&inflated,
    ...
    if(inflated.size()<=128*128*sizeof(char)) {
        if(error_info!=nullptr) {
            *error_info="The inflated file content is invalid. The file size is "+std::to_string(inflated.size())+" which is less than 128*128, it can't contain the map content.";
        }
        return nullptr;
    }
    ...
    constexpr uint8_t feature[]={0x07 ,0x00 ,0x06,
                                 0x63 ,0x6F ,0x6C ,0x6F ,0x72 ,0x73 ,0x00 ,0x00 ,0x40 ,0x00};
    ...
    if(error_info!=nullptr) {
        *error_info="Failed to find map data array named color. The map data file may be invalid.";
    }
...
bool process_map_file(const char * filename,
    ...
    if((!std::filesystem::exists(filename))||(!std::filesystem::is_regular_file(filename))) {
        if(error_info!=nullptr) *error_info=std::string("File ")+filename+" doesn't exist, otherwise it is not a regular file.";
        return false;
    }
