...
AbstractBlock *SCL_EXPORT SCL_createBlock() { return AbstractBlock::create(); }
...
/// id of this block
const char *SCL_EXPORT SCL_getId(const AbstractBlock *t) { return t->getId(); }
...
bool SCL_EXPORT SCL_getDoGlow(const AbstractBlock *t) { return t->getDoGlow(); }
...
/// set block id
void SCL_EXPORT SCL_setId(AbstractBlock *t, const char *id) { t->setId(id); }
/// set first version
...
void SCL_EXPORT SCL_setDoGlow(AbstractBlock *t, bool dg) { t->setDoGlow(dg); }
...
void SCL_EXPORT SCL_setBurnable(AbstractBlock *t, bool b) { t->setBurnable(b); }
/// set if this block can be used in wall-map
void SCL_EXPORT SCL_setWallUseable(AbstractBlock *t, bool wu) {
  t->setWallUseable(wu);
}
/// let *b equal to *this
void SCL_EXPORT SCL_copyTo(AbstractBlock *t, AbstractBlock *b) { t->copyTo(b); }
/// set this block to air
void SCL_EXPORT SCL_clear(AbstractBlock *t) { t->clear(); }
/// replacement for operator delete
void SCL_EXPORT SCL_destroyBlock(AbstractBlock *t) { t->destroy(); }
...
const char *SCL_EXPORT SCL_getSCLVersion() { return Kernel::getSCLVersion(); }
...
// can do in nothing:
/// real size of Kernel
unsigned long long SCL_EXPORT SCL_kernelSize(Kernel *k) { return k->size(); }
/// revert to a previous step
void SCL_EXPORT SCL_decreaseStep(Kernel *k, step s) { k->decreaseStep(s); }
/// replacement for operator delete
void SCL_EXPORT SCL_destroyKernel(Kernel *k) { k->destroy(); }
...
/// get current step
step SCL_EXPORT SCL_queryStep(const Kernel *k) { return k->queryStep(); }
/// set map type and blocklist
bool SCL_EXPORT SCL_setType(Kernel *k, mapTypes a, gameVersion b,
                            const bool c[64], const AbstractBlock *d[64]) {
...
void SCL_EXPORT SCL_setRawImage(Kernel *k, const unsigned int *src, short rows,
                                short cols) {
...
void SCL_EXPORT SCL_makeTests(Kernel *k, const AbstractBlock **a,
                              const unsigned char *b, const char *c, char *d) {
  k->makeTests(a, b, c, d);
}
...
/// get image rows
short SCL_EXPORT SCL_getImageRows(const Kernel *k) { return k->getImageRows(); }
/// get image cols
short SCL_EXPORT SCL_getImageCols(const Kernel *k) { return k->getImageCols(); }
/// query if map is buildable in vanilla survival
bool SCL_EXPORT SCL_isVanilla(const Kernel *k) { return k->isVanilla(); }
/// query if map is a flat one
bool SCL_EXPORT SCL_isFlat(const Kernel *k) { return k->isFlat(); }

// can do in converted:
/// construct 3D structure
bool SCL_EXPORT SCL_build(Kernel *k, compressSettings cs, unsigned short mAH,
                          glassBridgeSettings gbs, unsigned short bl,
                          bool fireProof, bool endermanProof) {
  return k->build(cs, mAH, gbs, bl, fireProof, endermanProof);
}

/// get converted image
void SCL_EXPORT SCL_getConvertedImage(const Kernel *k, short *rows, short *cols,
                                      unsigned int *dest) {
...
void SCL_EXPORT SCL_getConvertedMap(const Kernel *k, short *rows, short *cols,
                                    unsigned char *c) {
...
int SCL_EXPORT SCL_getHeight(const Kernel *k) { return k->getHeight(); }
/// get 3d structure's size
int SCL_EXPORT SCL_getXRange(const Kernel *k) { return k->getXRange(); }
/// get 3d structure's size
int SCL_EXPORT SCL_getZRange(const Kernel *k) { return k->getZRange(); }
...
void SCL_EXPORT SCL_setReportWorkingStatue(Kernel *k,
                                           void (*p)(void *, workStatues)) {
  k->setReportWorkingStatue(p);
}

#endif //  ifdef SLOPECRAFTL_CAPI

AiCvterOpt *SCL_EXPORT SCL_createAiCvterOpt() { return new AiCvterOpt; }
