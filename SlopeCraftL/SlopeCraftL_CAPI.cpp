...
/// get image rows
int SCL_EXPORT SCL_getImageRows(const Kernel *k) { return k->getImageRows(); }
/// get image cols
int SCL_EXPORT SCL_getImageCols(const Kernel *k) { return k->getImageCols(); }
...
void SCL_EXPORT SCL_exportAsLitematic(const Kernel *k, const char *TargetName,
  ...
  k->exportAsLitematic(TargetName, LiteName, RegionName, FileName);
}
...
void SCL_EXPORT SCL_setReportError(Kernel *k, void (*p)(void *, SCL_errorFlag,
                                                        const char *)) {
  k->setReportError(p);
}
