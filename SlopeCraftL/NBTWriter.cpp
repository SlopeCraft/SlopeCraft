...
template <typename T> void NBT::IE2BE(T &Val) {
  if (sizeof(T) <= 1)
    return;
  char *s = (char *)&Val;
...
template <typename T> T NBT::IE2BE(T *Val) {
  if (sizeof(T) <= 1)
    return *Val;
...
bool NBTWriter::isEmpty() { return (top == -1); }

bool NBTWriter::isFull() { return top >= TwinStackSize; }

bool NBTWriter::isListFinished() { return (Size[top] <= 0); }

char NBTWriter::readType() { return CLA[top]; }

bool NBTWriter::isInCompound() { return isEmpty() || (readType() == 0); }

bool NBTWriter::isInList() { return !isInCompound(); }

bool NBTWriter::typeMatch(char typeId) { return (readType() == typeId); }
...
int NBTWriter::writeSingleTag(char typeId, const char *Name, T value) {
  ...
  if (!isBE) {
    IE2BE(writeNameL);
    IE2BE(value); // value不需要读取，只需要写入
  }
  if (isInCompound()) //写入为完整的Tag
  {
    // qDebug()<<"写入为文件夹内的id"<<(short)typeId;
    File->write(&typeId, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
    ThisCount += realNameL;
...
int NBTWriter::writeLongDirectly(const char *Name, long long value) {
  ...
  if (isInCompound()) //写入为完整的Tag
  {
    ...
    File->write(&NBT::idLong, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
    ThisCount += realNameL;
    File->write((char *)&value, sizeof(long long));
    ThisCount += sizeof(long long);
  }
...
int NBTWriter::writeCompound(const char *Name) {
  ...
  if (isInCompound()) {
    File->write(&idCompound, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
...
int NBTWriter::writeListHead(const char *Name, char TypeId, int listSize) {
  ...
  if (isInCompound()) {
    ...
    File->write(&TypeId, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeListSize, sizeof(int));
    ThisCount += sizeof(int);
...
int NBTWriter::writeLongArrayHead(const char *Name, int arraySize) {
  ...
  if (!isBE) {
    IE2BE(writeNameL);
    IE2BE(writeArraySize);
  }

  if (isInCompound()) {
    File->write(&idLongArray, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
    ...
    File->write((char *)&writeArraySize, sizeof(int));
    ThisCount += sizeof(int);
...
int NBTWriter::writeByteArrayHead(const char *Name, int arraySize) {
  ...
  if (isInCompound()) {
    File->write(&idByteArray, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
...
int NBTWriter::writeIntArrayHead(const char *Name, int arraySize) {
  ...
  if (isInCompound()) {
    File->write(&idIntArray, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
...
int NBTWriter::writeString(const char *Name, const char *value) {
  ...
  if (isInCompound()) {
    File->write(&idString, sizeof(char));
    ThisCount += sizeof(char);
    File->write((char *)&writeNameL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(Name, realNameL);
    ThisCount += realNameL;
    File->write((char *)&writeValL, sizeof(short));
    ThisCount += sizeof(short);
    File->write(value, realValL);
    ThisCount += realValL;
...
char NBTWriter::CurrentType() { return readType(); }

int NBTWriter::emergencyFill() {
  if (!allowEmergencyFill)
    return 0;
  if (isEmpty())
    return 0;
  int ThisCount = 0;
  ...
  ThisCount +=
      writeString("TokiNoBug'sWarning",
                  "There's sth wrong with ur NBTWriter, the file format is "
                  "completed automatically instead of manually.");
  std::cerr << "emergency fill finished.\n";
  return ThisCount;
}
unsigned long long NBTWriter::getByteCount() { return ByteCount; }
