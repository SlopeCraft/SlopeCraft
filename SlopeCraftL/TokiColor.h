...
inline uint32_t getA(ARGB argb) noexcept { return (argb >> 24); }

inline uint32_t getR(ARGB argb) noexcept { return (argb & 0x00FF0000) >> 16; }

inline uint32_t getG(ARGB argb) noexcept { return (argb & 0x0000FF00) >> 8; }

inline uint32_t getB(ARGB argb) noexcept { return (argb & 0x000000FF); }
...
public:
  ...
  float sideSelectivity
      [2]; //记录与result的深度值不同的两个有损优化候选色选择系数（升序排列），Depth=3时无效
  unsigned char sideResult
      [2]; //记录与result的深度值不同的两个有损优化候选色（升序排列），Depth=3时无效
