    ...
    ui(new Ui::BatchUi)
{
  ...
  connect(ui->setTypeData, &QRadioButton::clicked, this,
          &BatchUi::onTaskTypeChanged);
  connect(ui->setTypeLite, &QRadioButton::clicked, this,
          &BatchUi::onTaskTypeChanged);
  connect(ui->setTypeStructure, &QRadioButton::clicked, this,
          &BatchUi::onTaskTypeChanged);
  connect(ui->setDataFolder, &QLineEdit::textEdited, ui->setDataFolder,
          &QLineEdit::textChanged);
  connect(ui->setDataFolder, &QLineEdit::textChanged, this,
          &BatchUi::checkExecutable);
}
...
void BatchUi::on_BtnBrowseDataFolder_clicked() {
  QString path = QFileDialog::getExistingDirectory(this, tr("选择输出文件夹"),
...
SlopeCraft::Kernel *BatchUi::kernel() const { return wind()->kernelPtr(); }
...
void BatchUi::on_BtnExecute_clicked() {
  ...
  for (size_t idx = 0; idx < taskBoxes.size(); idx++) {
    ...
    QString prefix = tr("批量处理中：") + QString::number(idx + 1) + " / " +
    ...
    if (TaskBox::taskType != TaskType::Data) {
      ...
      wind()->ui->InputDataIndex->setText(
          QString::number(curTask->begSeqNum()));
